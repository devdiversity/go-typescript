...
func ParseEndpoint(source string, file string, line int) TSEndpoint {
	...
	for _, v := range a {
		...
		if len(t) < 2 || strings.Trim(t[1], " ") == "" {
			exiterrorf.ExitErrorf(errors.New(fmt.Sprintf("Worong Endpoint: %s", s)))
		}
		if len(t) == 2 {
			...
			exiterrorf.ExitErrorf(errors.New(fmt.Sprintf("Worong Endpoint props: %s", s)))
		}
	}

	if endpoint.Method != "POST" && endpoint.Method != "GET" {
		exiterrorf.ExitErrorf(errors.New(fmt.Sprintf("Worong Endpoint method: %s", s)))
	}

	if endpoint.Method == "GET" && n < 4 {
		exiterrorf.ExitErrorf(errors.New(fmt.Sprintf("Worong Endpoint number of props: %s", s)))
	}
...
func (e *TSEndpoint) VerifyTypes(info TSInfo, p string) {
	...
	if e.Request != "" {
		if len(a) == 2 {
			if !info.find(a[0], a[1]) {
				exiterrorf.ExitErrorf(errors.New(fmt.Sprintf("Worong Endpoint request: %s AT %s Line: %d ", e.Request, e.File, e.Line)))
			}
		}
	...
	if len(a) == 2 {
		if !info.find(a[0], a[1]) {
			exiterrorf.ExitErrorf(errors.New(fmt.Sprintf("Worong Endpoint response: %s AT %s Line: %d ", e.Request, e.File, e.Line)))
		}
	}
	if len(a) == 1 && !isNativeType(a[0]) {
		if !info.find(p, a[0]) {
